...
/*
 * log : pg개발이나 운영시 문제점을 추적, 운영상태를 모티터링
 * syso는 로그 확인 불가능, 로그를 기록하는 방법사용
 * log4j은 단계  trace> debug> info> warn > error> fatal
 * fatal : 심각한 에러 프로그램 실행 x
 * error : 문제가 발생하면 sw 일부 기능을 수행되지 못함 
 * warn : sw가 동장은 하지만 예상치 못한 변수가 가까운 미래에 발생하는 경고
 * info : 작업이 정상 작동하고 있다는것을 메세지로 통해 확인하고 싶은 경우 사용
 * debug : 문제 발생시 진단하고 싶은 때 필요한 정보 기록
 * trace : 디버그 레벨이 낮아 광범위한것을 해결하기위해 좀더 상세한 이베트를 나타냄
 */
